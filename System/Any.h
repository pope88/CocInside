...
/// these cases.
{
	...
	if (!result) throw std::exception(/*"Failed to convert between const Any types"*/);
	return *result;
}
...
{
	ValueType* result = AnyCast<ValueType>(const_cast<Any*>(&operand));
	if (!result) throw std::exception(/*"RefAnyCast: Failed to convert between const Any types"*/);
	return *result;
}
...
{
	ValueType* result = AnyCast<ValueType>(&operand);
	if (!result) throw std::exception(/*"RefAnyCast: Failed to convert between Any types"*/);
	return *result;
}
