...
#ifdef SYSTEM_SYSTEM_WINDOWS

#include "Win32/PathImpl.cpp"

#else

#include "Unix/PathImpl.cpp"
...
{
	return priv::PathImpl::currentImpl();
}

	
std::string Path::home()
{
	return priv::PathImpl::homeImpl();
}

	
std::string Path::temp()
{
	return priv::PathImpl::tempImpl();
}


std::string Path::null()
{
	return priv::PathImpl::nullImpl();
}

	
std::string Path::expand(const std::string& path)
{
	return priv::PathImpl::expandImpl(path);
}


void Path::listRoots(std::vector<std::string>& roots)
{
	priv::PathImpl::listRootsImpl(roots);
}
			...
			if (name.empty())
			{
				if (it != end && *it == '[')
				{
					...
					if (it != end)
					{
						...
						if (it == end) throw std::exception();
						++it;
						if (it != end && *it == '[')
						{
							if (!_absolute) throw std::exception();
							++it;
							if (it != end && *it == '.') throw std::exception();
